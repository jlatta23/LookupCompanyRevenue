import csv
from smallbusinessapp.models import StateIndustryRevenueAverages
...
class Command(BaseCommand):
    def handle(self, **options):
        data_path = Path("C:/WebDevelopment/businesslookup_research/data/avg_revenue_by_state_and_industry.csv")
        start_ind = 0
        end_ind = 100000
        cur_ind = 0
        num_cur_interval = 0
        write_interval = 2000
        in_mem_data = []
        StateIndustryRevenueAverages.objects.all().delete()
            ...
            for row in csv_reader:
                if cur_ind >= start_ind and cur_ind < end_ind:
                    ...
                    if num_cur_interval > write_interval:
                        ...
                        StateIndustryRevenueAverages.objects.bulk_create(in_mem_data)
                        print('finished bulk create')
                        time.sleep(0.65)
                        num_cur_interval = 0
                        in_mem_data = []
                    obj = StateIndustryRevenueAverages(
