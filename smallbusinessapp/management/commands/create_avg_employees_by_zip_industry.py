import csv
from smallbusinessapp.models import ZipCodeIndustryEmployeesAverages
...
class Command(BaseCommand):
    def handle(self, **options):
        data_path = Path("C:/WebDevelopment/businesslookup_research/data/avg_employees_by_zip_and_industry.csv")
        start_ind = 0
        end_ind = 3200000
        cur_ind = 0
        num_cur_interval = 0
        ...
        in_mem_data = []
        ZipCodeIndustryEmployeesAverages.objects.all().delete()
            ...
            for row in csv_reader:
                if cur_ind >= start_ind and cur_ind < end_ind:
                    ...
                    if num_cur_interval > write_interval:
                        ...
                        ZipCodeIndustryEmployeesAverages.objects.bulk_create(in_mem_data)
                        print('finished bulk create')
                        time.sleep(0.65)
                        num_cur_interval = 0
                        in_mem_data = []
                    obj = ZipCodeIndustryEmployeesAverages(
